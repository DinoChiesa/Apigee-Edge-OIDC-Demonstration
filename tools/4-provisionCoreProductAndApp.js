...
// 4-provisionCoreProductAndApp.js
// ------------------------------------------------------------------
// provision an Apigee Edge Developer, API Product, and App for OIDC Core.
//
// last saved: <2018-August-28 17:40:31>
...
const fs            = require('fs'),
      ...
      settings      = {
        product : {
          name : 'OIDC-Core-Product',
          proxies : ['oidc-core']
        },
        app : {
          name : 'OIDC-Core-App',
          logoUrl : 'https://www.marketingtechblog.com/wp-content/uploads/2010/06/example-logo-660x330.png',
          callbackUrl : 'http://dinochiesa.github.io/openid-connect/callback-handler.html',
          keyExpiry : '180d',
        },
        developer : {
          email : 'oidc-core-example@example.com',
          last : 'Example',
          first : 'OIDCCore'
        }
      },
      getopt        = new Getopt(common.commonOptions.concat([
        ['U', 'loginUrl=ARG', 'required. URL for login.']
      ])).bindHelp();
    ...
    org.products.get(function(e, productsList){
      ...
      if ( ! productsList.find(x => (x == settings.product.name))) {
        var options = {
              ...
              approvalType : 'auto',
              attributes: { "login-url": opt.options.loginUrl }
            };
    ...
    org.developerapps.get({email: settings.developer.email}, function(e, appList){
      ...
      if ( ! appList.find(x => (x == settings.app.name))) {
        var options = {
              ...
              callbackUrl : settings.app.callbackUrl,
              attributes: { "logo-url": settings.app.logoUrl }
            };
